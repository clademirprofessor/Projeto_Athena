...
global CLI_MODEL_PREF, SKIP_MEM_CONFIRM, MODEL_RAM_THRESHOLD_MB
CLI_MODEL_PREF = "full" # 'small', 'full', or None
CLI_TTS_MODE = "online"  # TtsMode or None  online, offline, auto
SKIP_MEM_CONFIRM = False
...
#Tempo de espera entre reconhecimentos de comando
COMMAND_RECOGNITION_DELAY = 2.0  # segundos
# Tempo máximo para esperar o TTS terminar (segundos)
TTS_WAIT_TIMEOUT = 5.0
...
def choose_model():
    """Escolhe entre modelo leve e completo conforme existência, RAM disponível e preferência CLI"""
    ...
    # Honor CLI preference if provided
    if CLI_MODEL_PREF == "small":
        if small_exists:
            print("Preferência CLI: usar modelo leve.")
            return SMALL_MODEL_DIR
        elif full_exists:
            print("Preferência CLI: modelo leve não encontrado, usando completo.")
            return FULL_MODEL_DIR
        else:
            print("Nenhum modelo encontrado conforme preferência CLI.")
            sys.exit(1)
    if CLI_MODEL_PREF == "full":
        if full_exists:
            print("Preferência CLI: usar modelo completo.")
            # if skipping mem confirm, return without asking
            if avail_mb is not None and avail_mb < MODEL_RAM_THRESHOLD_MB and not SKIP_MEM_CONFIRM:
                ...
                if confirm not in ("y", "yes"):
                    print("Abortando por solicitação do usuário.")
                    sys.exit(1)
            return FULL_MODEL_DIR
        elif small_exists:
            print("Preferência CLI: modelo completo não encontrado, usando leve.")
            return SMALL_MODEL_DIR
        else:
            print("Nenhum modelo encontrado conforme preferência CLI.")
            sys.exit(1)

    # existing interactive logic if no CLI preference
    if full_exists and small_exists:
        ...
        if choice == "2":
            ...
            if avail_mb is not None and avail_mb < MODEL_RAM_THRESHOLD_MB:
                if SKIP_MEM_CONFIRM:
                    print("SKIP_MEM_CONFIRM ativado: pulando confirmação de memória e usando modelo completo.")
                    return FULL_MODEL_DIR
                confirm = input(f"Atenção: memória disponível ~{avail_mb}MB < {MODEL_RAM_THRESHOLD_MB}MB. Modelo completo pode não caber na RAM. Confirmar uso do completo? [y/N]: ").strip().lower()
                if confirm not in ("y", "yes"):
...
def load_tts_config():
    """Load TTS configuration from file or create with defaults
       If CLI_TTS_MODE is set, return that mode (override)."""
    global CLI_TTS_MODE
    if CLI_TTS_MODE is not None:
        return CLI_TTS_MODE
...
def load_model_and_measure(model_path):
    ...
    Respeita SKIP_MEM_CONFIRM para pular prompts.
    """
    ...
    if delta is not None and delta > MODEL_RAM_THRESHOLD_MB:
        ...
        if SKIP_MEM_CONFIRM:
            print("SKIP_MEM_CONFIRM ativado: ignorando confirmação de memória e prosseguindo.")
        else:
...
def speak(text: str, mode: TtsMode = None):
    ...
    if mode in (TtsMode.AUTO, TtsMode.OFFLINE):
        ...
        if espeakng:
            subprocess.run([espeakng, "-v", "pt", text])
            return
        if espeak:
            subprocess.run([espeak, "-v", "pt", text])
            return
        ...
        if resp:
            try:
                ...
                if _HAS_PSUTIL:
                    ...
                    for p in procs:
                        try:
                            p.wait(timeout=globals().get("TTS_WAIT_TIMEOUT", 5))
    ...
    try:
        with sd.RawInputStream(samplerate=SAMPLE_RATE, blocksize=8000, dtype='int16',
            ...
            while True:
                ...
                time.sleep(globals().get("COMMAND_RECOGNITION_DELAY", 1.0))
...
# Adiciona parsing de argumentos do terminal e popula variáveis globais antes de main()
def parse_cli_args():
    """Parse CLI args to override TTS mode, model choice and memory confirmations"""
    #global CLI_TTS_MODE, CLI_MODEL_PREF, SKIP_MEM_CONFIRM

    p = argparse.ArgumentParser(description="test_vosk_microfone_serial_talkback")
    p.add_argument("--tts-mode", choices=("auto", "online", "offline"), help="Modo TTS: auto|online|offline (override config file)")
    p.add_argument("--model", choices=("auto", "small", "full"), default="auto", help="Modelo de linguagem padrão a usar se disponível")
    p.add_argument("--skip-mem-confirm", action="store_true", help="Pular confirmações de uso de memória (útil para scripts/CI)")
    p.add_argument("--model-ram-threshold-mb", type=int, help="Ajustar limite de RAM para alertas (MB)")
    args = p.parse_args()

    if args.tts_mode:
        if args.tts_mode == "auto":
            CLI_TTS_MODE = TtsMode.AUTO
        elif args.tts_mode == "online":
            CLI_TTS_MODE = TtsMode.ONLINE
        elif args.tts_mode == "offline":
            CLI_TTS_MODE = TtsMode.OFFLINE

    if args.model and args.model != "auto":
        CLI_MODEL_PREF = args.model  # 'small' or 'full'

    if args.skip_mem_confirm:
        SKIP_MEM_CONFIRM = True

    if args.model_ram_threshold_mb:
        globals()["MODEL_RAM_THRESHOLD_MB"] = int(args.model_ram_threshold_mb)
