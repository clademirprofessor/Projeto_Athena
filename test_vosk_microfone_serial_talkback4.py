    ...
    if full_exists and small_exists:
        ...
        if choice == "2":
            ...
            if avail_mb is not None and avail_mb < 2000:
                confirm = input(f"Atenção: memória disponível ~{avail_mb}MB < 2000MB. Modelo completo pode não caber na RAM. Confirmar uso do completo? [y/N]: ").strip().lower()
        ...
        if avail_mb is not None and avail_mb < 2000:
            confirm = input(f"Atenção: memória disponível ~{avail_mb}MB < 2000MB. Modelo completo pode não caber na RAM. Deseja prosseguir? [y/N]: ").strip().lower()
            if confirm not in ("y", "yes"):
...
def try_send_serial(ser, text):
    ...
    try:
        ...
        # 1. Envio do comando
        print(f"Enviando para serial: '{text}'")
        ser.write((text + "\n").encode("utf-8"))
        ser.flush()

        # 2. Espera resposta com timeout
        start = time.time()
        resp = b""
        while time.time() - start < SERIAL_RESPONSE_TIMEOUT:
            if ser.in_waiting:
                ...
                if line:
                    resp += line
                    # Se linha termina com newline, assumir fim da resposta
                    if resp.endswith(b"\n") or resp.endswith(b"\r\n"):
                        break
        ...
        speak("Erro ao comunicar com Arduino")
...
# No loop principal, aguardar conclusão antes de aceitar novo comando:
def check_arduino_communication(ser):
    """Verifica se a comunicação com o Arduino está funcionando."""
    if ser is None:
        return False
    try:
        ser.write(b'ping\n')
